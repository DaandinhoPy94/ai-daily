-- First create the media asset for the Grok image
...
-- Then create the Grok article with the media asset as hero image
INSERT INTO articles (
  ...
  'Hoe Elon Musk Grok naar zijn eigen beeld vormt',
...
<p>De ontwikkeling van Grok markeert een belangrijk moment in de AI-evolutie, waarbij de grens tussen technologie en persoonlijkheid steeds verder vervaagt.</p>',
  ...
  (SELECT id FROM media_assets WHERE path = 'src/assets/grok-ai-interface.jpg'),
  NOW() - INTERVAL '2 hours',
  NOW() - INTERVAL '2 hours',
  NOW() - INTERVAL '2 hours'
);

-- Add individual article views for the new article (87 views)
DO $$
DECLARE
    new_article_id uuid;
    i integer;
BEGIN
    -- Get the article ID
    SELECT id INTO new_article_id FROM articles WHERE slug = 'elon-musk-grok-eigen-beeld';
    
    -- Insert individual view records
    FOR i IN 1..87 LOOP
        INSERT INTO article_views (article_id, viewed_at)
        VALUES (
            new_article_id,
            NOW() - (INTERVAL '1 hour' * (87 - i)) - (INTERVAL '1 minute' * RANDOM() * 60)
        );
    END LOOP;
END $$;